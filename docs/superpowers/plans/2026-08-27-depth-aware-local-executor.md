...
**Tech Stack:** Isaac Gym Preview 4, PyTorch, `legged_gym`, existing DWL/RSL-RL PPO components, CUDA, Python unittest/pytest.
...
- `DepthCameraMixin.capture_depth() -> torch.Tensor` returns `[N, 8, 32]` and records `depth_backend` as `"isaacgym"` or `"fallback"`.
- `DepthCameraMixin._apply_depth_noise(depth: torch.Tensor) -> torch.Tensor` preserves finite values and `[0, 1]` bounds.

- [ ] **Step 1: Write failing pure tests**

Test that normalized values clamp to `[0, 1]`, invalid/NaN camera values become far-plane values, and a synthetic symmetric corridor produces symmetric left/right columns. Test that the backend label is explicit.
...
```bash
python -m pytest -q legged_gym/tests/test_depth_camera_math.py
```
...
```python
depth_backend = "isaacgym"  # actual IMAGE_DEPTH path
depth_backend = "fallback"  # explicit headless ray/AABB path
```

Do not silently switch from a requested real camera to fallback during formal evaluation; raise a clear `RuntimeError` instead.

- [ ] **Step 4: Run the focused tests and compile the mixin**

```bash
python -m pytest -q legged_gym/tests/test_depth_camera_math.py
python -m py_compile legged_gym/envs/rotunbot/maze/rotunbot_maze_camera.py
```
...
```bash
python -m pytest -q legged_gym/tests/test_local_goal.py
```
...
**Interfaces:**
- Constructor accepts the existing runner arguments `num_short_obs`, `num_proprio_obs`, `num_critic_obs`, `num_actions` plus `depth_height=8`, `depth_width=32`, and PPO policy settings.
...
- `act_inference(torch.zeros(4, 272))` returns `[4, 2]`; `evaluate(torch.zeros(4, 16))` returns `[4, 1]`.
...
```bash
python -m pytest -q legged_gym/tests/test_actor_critic_depth_local.py
```

- [ ] **Step 3: Implement V0**

Use three stride-2 convolutions `1→16→32→64`, an image projection to 64 dimensions, a state MLP `16→64→64`, fusion to 128 dimensions, and actor layers `128→256→128→2`. Keep the critic as a separate MLP over the configured 16-dimensional privileged state. Use a newly initialized diagonal Normal action distribution and do not import or load any checkpoint.

- [ ] **Step 4: Run tests and compile**

```bash
python -m pytest -q legged_gym/tests/test_actor_critic_depth_local.py
python -m py_compile legged_gym/dwl/actor_critic_depth_local.py
```
...
- Environment state: `global_goal_xy_world`, `active_local_goal_xy_world`, `active_local_goal_xy_robot`, `waypoint_reached`, `global_goal_reached`, `waypoint_changed`, `prev_local_goal_dist`, and `depth_backend`.
- `get_observations()` returns `[N, 272]`; privileged observations return `[N, 16]`.
...
Use `class RotunbotMazeLocalDepth(DepthCameraMixin, RotunbotMaze)`. Reuse the existing maze actors, collision geometry, physics, and planar controller. Maintain global and active local goals separately; for Stage 0–3 the active local goal is also the episode target, while Stage 4 uses the BFS waypoint state.
...
```bash
python -m pytest -q legged_gym/tests/test_depth_local_observation.py
python -m py_compile legged_gym/envs/rotunbot/maze/rotunbot_maze_local_depth.py legged_gym/envs/rotunbot/maze/rotunbot_maze_local_depth_config.py
```
...
```bash
python -m pytest -q legged_gym/tests/test_depth_local_rewards.py legged_gym/tests/test_waypoint_switching.py
```

- [ ] **Step 3: Implement the state machine and rewards**

Use local progress scale 3.0, local reach scale 20.0, wall penalty scale 0.5, collision scale -20.0, and action-rate scale -0.01. For Stage 0–3, active-goal reach sets episode success. For Stage 4, active-goal reach calls `_advance_active_waypoint()` and only `global_goal_reached` terminates successfully. Ensure the first transition after a switch has `waypoint_changed=True` and contributes zero progress.

Select active waypoints from BFS candidates only when their distance, lateral displacement, and bearing are compatible with the measured planar action response and camera coverage. Stage 0 starts with `g_x` 0.4–1.5 m and `|g_y|` ≤ 0.6 m.

- [ ] **Step 4: Run reward/switching tests and existing map tests**

```bash
python -m pytest -q legged_gym/tests/test_depth_local_rewards.py legged_gym/tests/test_waypoint_switching.py legged_gym/tests/test_oracle_local_subgoal.py legged_gym/tests/test_rotunbot_maze_map.py
```
...
**Interfaces:**
- Both evaluators record `depth_backend`, local success, global success, waypoint reach count, collision, timeout, final distance, path length, and completion time.
...
```bash
python -m pytest -q legged_gym/tests/test_depth_local_evaluation.py
```

- [ ] **Step 3: Implement fixed evaluation grids**

Use the Stage 0 open-space grid with random initial yaw and reachable local targets, then the corridor/corner cases, and finally a fixed maze manifest. Include `depth_backend` in every JSON report and never silently replace a requested camera backend with fallback.

- [ ] **Step 4: Run evaluator unit tests and compile checks**

```bash
python -m pytest -q legged_gym/tests/test_depth_local_evaluation.py
python -m py_compile legged_gym/scripts/evaluate_depth_local.py legged_gym/scripts/evaluate_depth_local_maze.py
```
...
The smoke must instantiate two environments, reset and step them, verify observations `[2, 272]`, finite rewards, depth bounds, action output `[2, 2]`, no historical checkpoint path, and explicit fallback/camera backend.

- [ ] **Step 2: Run the smoke before long training**

```bash
CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python legged_gym/scripts/smoke_depth_local.py --headless --sim_device=cuda:0 --rl_device=cuda:0
```

- [ ] **Step 3: Run one PPO update with a new policy**

```bash
CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python legged_gym/scripts/train.py --task rotunbot_maze_local_depth --headless --num_envs 2 --max_iterations 1
```
...
Run Stage 0 first. Only after its local success gate of 90% passes, evaluate the bounded corridor, feasible-waypoint corner, planner-provided detour, and full maze. Validate representative cases with real Isaac Gym `IMAGE_DEPTH` after fallback smoke succeeds.
...
- [ ] The new environment returns exactly 272 actor values and 16 critic values.
