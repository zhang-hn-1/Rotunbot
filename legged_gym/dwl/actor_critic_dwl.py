...
class ActorCriticDWL(nn.Module):
    ...
    def __init__(self,  num_short_obs,
        ...
        # Optional action-space adapter for controller changes.  It preserves
        # the checkpoint's 2-D policy interface while allowing a new
        # DIRECT_VP actuator to learn a small action remapping.  Identity
        # initialization makes an old checkpoint behaviorally unchanged at
        # the start of an experiment.
        self.action_adapter_enabled = bool(action_adapter_enabled)
        self.action_adapter_train_only = bool(action_adapter_train_only)
        if self.action_adapter_enabled:
            self.action_adapter = nn.Linear(num_actions, num_actions)
            nn.init.eye_(self.action_adapter.weight)
            nn.init.zeros_(self.action_adapter.bias)
        else:
        ...
        if self.action_adapter_enabled and self.action_adapter_train_only:
            for module in (self.actor, self.long_history, self.critic):
    ...
    def act_inference(self, observations):
        ...
        if self.action_adapter is not None:
            actions_mean = self.action_adapter(actions_mean)
        return actions_mean
