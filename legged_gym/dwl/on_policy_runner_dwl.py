...
class DWLOnPolicyRunner:
    ...
    def log(self, locs, width=80, pad=35):
        ...
        log_string += (f"""{'-' * width}\n"""
                       ...
                       f"""{'ETA:':>{pad}} {self.tot_time / (locs['it'] + 1) * remaining_iterations:.1f}s\n""")
        print(log_string)
    ...
    def load(self, path, load_optimizer=True):
        # Checkpoints are commonly saved from CUDA training.  Respect the
        # runner device so the same checkpoint can be smoke-tested or resumed
        # on a CPU-only machine.
        loaded_dict = torch.load(path, map_location=self.device)
        # New controller experiments may add an identity-initialized action
        # adapter while keeping the original policy dimensions.  Load the
        # legacy weights and retain the adapter's initialization when its
        # keys are absent from the old checkpoint.
        ...
        if load_optimizer:
            try:
                ...
                # A newly added adapter changes the optimizer parameter set;
