...
class PPODWL:
    ...
    def compute_returns(self, last_critic_obs):
        # The rollout is collected under torch.inference_mode(), so the final
        # observation can be an inference tensor.  GAE is computed after that
        # context ends; clone it into a regular tensor before evaluating the
        # critic, otherwise PyTorch 1.10 raises when Linear tries to save the
        # inference input for backward.
        last_critic_obs = last_critic_obs.clone().detach()
        with torch.no_grad():
