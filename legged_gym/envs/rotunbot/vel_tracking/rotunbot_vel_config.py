...
class RotunbotVelCfg( LeggedRobotCfg ):
    ...
    class control( LeggedRobotCfg.control ):
        # Direct joint-command experiment:
        #   action[0] -> joint1 angular-velocity target
        #   action[1] -> joint2 position target
        # The old custom torque path is still available with control_type='R'.
        control_type = 'DIRECT_VP'
        ...
        # Scales/limits for the direct command path.  These match the
        # velocity/position command convention used by rotunbot_real.py.
        direct_velocity_scale = 1.0       # normalized action -> rad/s
        direct_velocity_limit = 6.0       # rad/s
        direct_position_scale = 0.5       # normalized action -> rad
        direct_position_limit = 0.5236    # rad
        # ********速度控制******** 
...
class RotunbotVelCfgPPO(LeggedRobotCfgPPO):
    ...
    class runner(LeggedRobotCfgPPO.runner):
        run_name = ''
        experiment_name = 'rotunbot_vel_direct_vp'
